...
class AntColonyTOP:
    def __init__(self, start_point, end_point, clients, m: int, L: float, debug=False):
        ...
        self.max_iterations = min(500, n * 10)  # Increased iterations for better convergence
        
        # ACO parameters - adjusted based on literature
        ...
        self.local_search_freq = 10  # frequency of local search application
        
        # Initialize pheromone trails
        self.tau_max = 1.0  # maximum pheromone value
    ...
    def _initialize_pheromones(self):
        """Initialize pheromone matrix with maximum value."""
        tau = {}
    ...
    def _initialize_heuristic(self):
        """Initialize heuristic information combining distance and profit."""
        eta = {}
        all_points = [self.start_point] + self.clients + [self.end_point]
        ...
        for i in all_points:
            ...
            for j in all_points:
                if i != j:
                    ...
                    # Normalize profit and combine with distance
    ...
    def _select_next_client(self, ant_route, current, available_clients, current_time):
        """Select next client using pseudo-random proportional rule."""
        ...
        if random.random() < self.q0:  # Exploitation
            max_val = float('-inf')
            chosen = self.end_point
            for client in feasible:
                val = (self.tau[current][client]**self.alpha * 
        ...
        # Exploration
        total = sum(self.tau[current][client]**self.alpha * 
                   self.eta[current][client]**self.beta 
                   for client in feasible)
        if total == 0:
        ...
        for client in feasible:
            prob = ((self.tau[current][client]**self.alpha * 
                    self.eta[current][client]**self.beta) / total)
            probabilities.append((client, prob))

        # Roulette wheel selection
        r = random.random()
        cum_prob = 0
        for client, prob in probabilities:
            cum_prob += prob
            if cum_prob >= r:
                return client
        return feasible[-1]
    ...
    def _construct_route(self, ant_id):
        """Construct a single route for an ant."""
        ...
        while available:
            ...
            self.tau[route[-2]][route[-1]] = ((1 - self.rho) * 
                                            self.tau[route[-2]][route[-1]] + 
                                            self.rho * self.tau_min)
        
        route.append(self.end_point)
        return route

    def _construct_solution(self):
        """Construct complete solution with multiple routes."""
        solution = []
    ...
    def _local_search(self, solution):
        """Apply 2-opt and swap operators to improve solution."""
        improved = True
        while improved:
            improved = False
            ...
            for route_idx, route in enumerate(solution):
                ...
                best_distance = self._calculate_route_length(route)
                for i in range(1, len(route)-2):
                    for j in range(i+1, len(route)-1):
                        new_route = route[:i] + list(reversed(route[i:j+1])) + route[j+1:]
                        new_distance = self._calculate_route_length(new_route)
                        
                        if new_distance < best_distance and self._is_feasible_route(new_route):
                ...
                for j in range(i+1, len(solution)):
                    for pos1 in range(1, len(solution[i])-1):
                        for pos2 in range(1, len(solution[j])-1):
                            # Try swapping clients
                            new_route1 = solution[i][:pos1] + [solution[j][pos2]] + solution[i][pos1+1:]
                            new_route2 = solution[j][:pos2] + [solution[i][pos1]] + solution[j][pos2+1:]
                            
                            if (self._is_feasible_route(new_route1) and 
                                self._is_feasible_route(new_route2)):
                                old_profit = (self._calculate_route_profit(solution[i]) + 
                                            self._calculate_route_profit(solution[j]))
                                new_profit = (self._calculate_route_profit(new_route1) + 
                                            self._calculate_route_profit(new_route2))
    ...
    def _calculate_route_length(self, route):
        ...
        return sum(math.sqrt((route[i].x - route[i+1].x)**2 + 
                           (route[i].y - route[i+1].y)**2)
                  for i in range(len(route)-1))

    def _calculate_route_profit(self, route):
        """Calculate total profit of a route."""
        return sum(client.profit for client in route[1:-1])

    def _is_feasible_route(self, route):
        """Check if route satisfies time constraint."""
        return self._calculate_route_length(route) <= self.L

    def _update_pheromones(self, solutions, qualities):
        """Update pheromone trails using MMAS approach."""
        # Evaporation
        for i in self.tau:
            for j in self.tau[i]:
        ...
        best_solution = solutions[np.argmax(qualities)]
        best_quality = max(qualities)
        
        for route in best_solution:
            for i in range(len(route)-1):
                deposit = 1.0 / (1.0 + self._calculate_route_length(route))
    ...
    def _calculate_solution_quality(self, solution):
        """Calculate solution quality considering multiple objectives."""
        if not solution:
            return 0
        
        total_profit = sum(self._calculate_route_profit(route) for route in solution)
        total_distance = sum(self._calculate_route_length(route) for route in solution)
        
        if any(self._calculate_route_length(route) > self.L for route in solution):
            return 0
        
        # Consider both profit and distance in fitness
        quality = total_profit * (1 - total_distance/(self.m * self.L))
        
        # Add bonus for number of clients served
    ...
    def _calculate_diversity(self, solutions):
        ...
        for i in range(len(solutions)):
            for j in range(i+1, len(solutions)):
                clients1 = {c.id for route in solutions[i] for c in route[1:-1]}
                clients2 = {c.id for route in solutions[j] for c in route[1:-1]}
    ...
    def solve(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            if self.debug and iteration % 10 == 0:
                ...
                print(f"  Pheromone range: [{min(pheromone_values):.3f}, {max(pheromone_values):.3f}]")
            
            # Update pheromones
            self._update_pheromones(solutions, qualities)
            
            # Early stopping check with minimum iterations
            if (iteration_without_improvement > 50 and iteration > 100):
                if self.debug:
                    print(f"\nEarly stopping at iteration {iteration}")
                    print(f"No improvement for {iteration_without_improvement} iterations")
                break
